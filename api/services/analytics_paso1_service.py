...
def _median_numeric(field: str, total_no_nulos: int) -> float | None:
    ...
    if total_no_nulos % 2 == 1:
        ...
        df = _to_df(raw)
        vals = [float(v) for v in (df[field].tolist() if not df.empty and field in df.columns else [])
                if v is not None]
...
def _num_stats(field: str) -> dict:
    raw = _q().select(
        f"max({field}) AS max_val", f"min({field}) AS min_val",
        f"avg({field}) AS avg_val", f"COUNT({field}) AS no_nulos",
    ).fetch()
    df = _to_df(raw)
    if df.empty:
...
def nulos_descripcion() -> dict:
    ...
    return {"pregunta": 3, "campo": A.DESCRIPCION, "total_registros": total,
            "no_nulos": no_nulos, "nulos": nulos,
...
def nulos_proceso() -> dict:
    ...
    no_nulos = int(df["no_nulos"].iloc[0]) if not df.empty and "no_nulos" in df.columns else 0
    nulos = total - no_nulos
    return {"pregunta": 4, "campo": A.PROCESO, "total_registros": total,
            "no_nulos": no_nulos, "nulos": nulos,
            "porcentaje_nulos": round(nulos / total * 100, 2) if total else 0}
...
def stats_tamano_archivo() -> dict:
    return {"pregunta": 8, "campo": A.TAMANNO_ARCHIVO, "unidad": "bytes",
            **_num_stats(A.TAMANNO_ARCHIVO)}


def stats_nit_entidad() -> dict:
    return {"pregunta": 9, "campo": A.NIT_ENTIDAD,
...
def rango_fecha_carga() -> dict:
    df = _to_df(_q().select(
        f"max({A.FECHA_CARGA}) AS max_fecha", f"min({A.FECHA_CARGA}) AS min_fecha",
    ).fetch())
    if df.empty:
        return {"pregunta": "10 y 11", "campo": A.FECHA_CARGA,
                "fecha_maxima": None, "fecha_minima": None, "rango_dias": None, "rango": None}
    ...
    try:
        ...
        pass
    return {"pregunta": "10 y 11", "campo": A.FECHA_CARGA,
            "fecha_maxima": max_f, "fecha_minima": min_f,
            "rango_dias": rango_dias, "rango": rango_str}


def documento_por_id(id_documento: int) -> dict:
    df = _to_df(_q().select(
        A.ID_DOCUMENTO, A.NOMBRE_ARCHIVO, A.FECHA_CARGA,
        A.EXTENSION, A.TAMANNO_ARCHIVO, A.ENTIDAD,
    ).where(f"{A.ID_DOCUMENTO} = {id_documento}").fetch())
    if df.empty:
        return {"pregunta": 12, "id_documento": id_documento,
                "resultado": None, "mensaje": "No encontrado"}
    row = df.iloc[0].to_dict()
    return {"pregunta": 12, "id_documento": id_documento,
